...
public class MyAdapter5 extends FirebaseRecyclerAdapter<Dataholder, MyAdapter5.Myviewholder> {



    public MyAdapter5(@NonNull final FirebaseRecyclerOptions<Dataholder> options) {
        super(options);
    }
            ...
            public void onClick(final View view) {
                ...
                builder.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(final DialogInterface dialogInterface, final int i) {
                        ...
                        FirebaseDatabase.getInstance().getReference().child("comusers")
                                .child(compcalldate).child("date").setValue(model.date);

                        FirebaseDatabase.getInstance().getReference().child("comusers")
                                .child(compcalldate).child("img").setValue(model.pimage);

                        FirebaseDatabase.getInstance().getReference().child("comusers")
                                .child(compcalldate).child("address").setValue(model.address);

                        FirebaseDatabase.getInstance().getReference().child("comusers")
                                .child(compcalldate).child("email").setValue(model.email);

                        FirebaseDatabase.getInstance().getReference().child("comusers")
                                .child(compcalldate).child("disease").setValue(model.disease);

                        FirebaseDatabase.getInstance().getReference().child("comusers")
                                .child(compcalldate).child("symptoms").setValue(model.symptoms);

                        FirebaseDatabase.getInstance().getReference().child("comusers")
                                .child(compcalldate).child("register").setValue(model.register);

                        FirebaseDatabase.getInstance().getReference().child("comusers")
                                .child(compcalldate).child("phone").setValue(model.phone);

                        FirebaseDatabase.getInstance().getReference().child("comusers")
                                .child(compcalldate).child("name").setValue(model.name)
                                .addOnSuccessListener(new OnSuccessListener<Void>() {
                                    @Override
                                    public void onSuccess(final Void aVoid) {
                                        FirebaseDatabase.getInstance().getReference().child("users")
    ...
    public Myviewholder onCreateViewHolder(@NonNull final ViewGroup parent, final int viewType) {
        final View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.singledata5, parent, false);
